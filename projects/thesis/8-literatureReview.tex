...
In the field of selective masking strategies, \citeA{posmaskinglearning} presented a training analysis based on POS masking focused on LM training and proposed a masking ratio decay to control the masking ratio for each POS category. 
The results showed that focusing on masking non-function words (ADJ, ADV, NOUN, PROPN, and VERB) in the later stages of training can encourage the LM model to develop a better understanding of context. 
...
Another work by \citeA{selective_masking} demonstrated that selectively masking infrequent words from the pre-training dataset during continued training enhances model performance on out-of-domain datasets. 
Furthermore, \cite{rf-curriculum-masking} 
In this work, we focus on analyzing the effect and behavior of the \acrshort{vl} model when trained with masking applied to each POS category.
