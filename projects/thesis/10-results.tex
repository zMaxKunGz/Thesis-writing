...
Among the non-function group, masking NOUN yields the best performance.
...
In the \acrshort{mlm} loss graph, we can see that \acrshort{pos} masking in the function group result in lower loss, while those in the non-function group show higher loss, and the random masking show the highest loss by the end of training.
...
By observing the \acrshort{mlm} loss graph, we find that non-function \acrshort{pos} are more difficult for the model to learn through the \acrshort{mlm} task, whereas function \acrshort{pos} are learned more quickly.
...
Random masking is treated as the baseline, and for clarity, the best-performing method in each task (excluding the baseline, and no masking) is highlighted in green, while the second-best method is highlighted in yellow.
...
Within the non‐function group, NOUN masking performed best, while VERB (69.13\%) and ADJ (69.09\%) achieved similar scores. ADV masking yielded the lowest performance (64.12\%), largely due to reduced accuracy in the Number and Other categories.
For function categories, DET and ADP masking achieved similar overall results (68.98\% and 68.96\%), with AUX (67.09\%) and PRON (66.55\%) performing lower.
...
For Flickr30K, we focus on exploring \acrshort{pos} masking for non-function \acrshort{pos} categories, as shown in Table \ref{tab:flickr30k-prob}.
